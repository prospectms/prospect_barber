...
class Profissional(TenantMixin, UnidadeMixin, db.Model):
    """Antigo `Barber`. Tabela mantida como `barbers` (ver nota em usuario.py).

    usuario_id agora é NULLABLE (relaxado na Fase 1-A): um profissional pode
    existir sem conta de login própria (ex.: cadastrado só para a agenda).
    Continua único quando presente — Postgres permite múltiplos NULL sob
    unique constraint, então nenhum profissional sem login colide entre si.
    """
    ...
    appointments = db.relationship("Agendamento", backref="barber", lazy="dynamic")
    ...
    def appointments_this_month(self) -> int:
        ...
        return self.appointments.filter(
            Agendamento.scheduled_date >= today.replace(day=1)
        ).count()
    ...
    def __repr__(self) -> str:
        return f"<Profissional {self.name}>"
